...
import {
  ...
  UserPlus, NotebookPen, PanelLeft, X, ChevronRight, ChevronLeft,
...
export function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex bg-background overflow-x-hidden app-command-surface">
      ...
      >
        {/* Logo */}
          <div className={`flex items-center h-16 border-b border-white/10 shrink-0 overflow-hidden transition-all duration-300 px-4 gap-2 ${navExpanded ? "md:px-4 md:gap-2" : "md:px-0 md:justify-center"}`}>
          ...
          <div className={`min-w-0 overflow-hidden transition-all duration-200 max-w-[140px] opacity-100 ${navExpanded ? "md:max-w-[140px] md:opacity-100" : "md:max-w-0 md:opacity-0"}`}>
            ...
            <p className="text-indigo-300/70 text-[10px] leading-none mt-0.5 truncate">Org Admin Portal</p>
            {userProfile?.orgName && (
        ...
        {userProfile?.orgId && (
           <div className={`overflow-hidden transition-all duration-200 max-h-0 opacity-0 ${navExpanded ? "md:max-h-0 md:opacity-0" : "md:max-h-12 md:opacity-100"}`}>

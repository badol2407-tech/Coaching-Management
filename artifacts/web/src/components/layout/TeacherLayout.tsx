...
import {
  ...
  GraduationCap, LogOut, CalendarDays, Settings, NotebookPen, Menu, X,
} from "lucide-react";
            ...
            return (
              <Link key={item.href} href={item.href}>
                ...
                >
                  ...
                  {active && <span className="ml-auto h-1.5 w-1.5 rounded-full bg-cyan-400 shadow-[0_0_6px_rgba(6,182,212,0.8)] shrink-0" />}
                </div>
              </Link>
            );
          })}
        ...
        <div className="border-t border-white/10 shrink-0">
          <div className="px-2 py-2">
            <Link href="/settings" onClick={closeDrawer}>
              <div className={`flex items-center gap-2.5 px-3 py-2 rounded-lg text-[13px] transition-colors cursor-pointer ${isSettings ? "text-cyan-300 bg-cyan-500/10" : "text-slate-400 hover:text-white hover:bg-white/5"}`}>
          ...
          <div className="border-t border-white/10 flex items-center gap-2.5 px-3 py-3">
            <div className="h-8 w-8 rounded-full bg-gradient-to-br from-cyan-400 to-blue-500 flex items-center justify-center text-white text-xs font-bold shadow-md shrink-0">

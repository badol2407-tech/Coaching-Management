...
export const getListStudentsQueryKey = (orgId?: string | null, search?: string, statusFilter?: string) => [orgId, "students", search ?? "", statusFilter ?? "all"];

export function useListStudents(params?: { search?: string; statusFilter?: "active" | "inactive" | "all" }) {
  const { userProfile } = useAuth();
  const orgId = userProfile?.orgId;
  return useQuery({
    queryKey: [orgId, "students", params?.search ?? "", params?.statusFilter ?? "all"],
    queryFn: async () => {
      if (!orgId) return [];
      ...
      if (params?.statusFilter && params.statusFilter !== "all") {
        const statusVal = params.statusFilter;
        rows = rows.filter((r) => (r.status ?? "active") === statusVal);
      }
...
/** List attendance with advanced client-side filters for history view */
export function useListAttendanceFiltered(params?: {
  date?: string;
  startDate?: string;
  endDate?: string;
  className?: string;
  ...
  const orgId = userProfile?.orgId;
  return useQuery({
    queryKey: [orgId, "attendance", "filtered", params],
    queryFn: async () => {
      if (!orgId) return [];
      const snap = await getDocs(orgCol(orgId, "attendance"));
      let rows = snap.docs.map(mapDoc) as any[];
      if (params?.date) rows = rows.filter((r) => r.date === params.date);
      if (params?.startDate) rows = rows.filter((r) => (r.date ?? "") >= params.startDate!);
      if (params?.endDate) rows = rows.filter((r) => (r.date ?? "") <= params.endDate!);
      if (params?.className) rows = rows.filter((r) => r.className === params.className);
      if (params?.batch) rows = rows.filter((r) => r.batch === params.batch);
...
/**
 * Mark attendance for a single student. Uses a deterministic doc ID
 * (studentId_date) to prevent duplicates — calling this twice for the
 * same student+date overwrites instead of creating a second record.
 */
export function useMarkAttendance() {
  ...
  return useMutation({
    mutationFn: async ({ data }: { data: Record<string, unknown> }) => {
      ...
      await setDoc(
        doc(db, "organizations", orgId, "attendance", docId),
...
/**
 * Bulk-mark attendance for an entire class/batch in one call.
 * Each record uses a deterministic doc ID to prevent duplicates.
 */
  ...
  return useMutation({
    mutationFn: async ({
      ...
      await Promise.all(
        records.map((r) =>
          setDoc(
            doc(db, "organizations", orgId, "attendance", `${r.studentId}_${date}`),
            { ...r, date, updatedAt: serverTimestamp() },
            { merge: true },
          ),
        ),
      );
...
/** Update an existing attendance record (e.g., to fix a wrong status in history). */
export function useUpdateAttendanceRecord() {
  ...
  return useMutation({
    mutationFn: async ({ id, data }: { id: string; data: Record<string, unknown> }) => {
      const orgId = userProfile?.orgId;
      if (!orgId) throw new Error("No org");
      await updateDoc(orgDocRef(orgId, "attendance", id), { ...data, updatedAt: serverTimestamp() });
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: [userProfile?.orgId, "attendance"] }),
  });
}
...
export function useDeleteAttendanceRecord() {
  ...
  return useMutation({
    mutationFn: async ({ id }: { id: string }) => {
      const orgId = userProfile?.orgId;
      if (!orgId) throw new Error("No org");
      await deleteDoc(orgDocRef(orgId, "attendance", id));
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: [userProfile?.orgId, "attendance"] }),
  });
}
...
/**
 * Monthly attendance summary grouped by student.
 * Returns one row per student with present/absent/late counts and %.
 */
export function useGetMonthlyAttendanceSummary(params?: {
  month?: string;
  className?: string;
  batch?: string;
}) {
  ...
  const orgId = userProfile?.orgId;
  return useQuery({
    queryKey: [orgId, "attendance", "monthly-summary", params],
    queryFn: async () => {
      if (!orgId || !params?.month) return [];
      const snap = await getDocs(orgCol(orgId, "attendance"));
      const rows = snap.docs.map(mapDoc) as any[];
      const monthRows = rows.filter((r) => (r.date ?? "").startsWith(params.month!));
      ...
      let result = Object.values(byStudent);
      if (params?.className) result = result.filter((r) => r.className === params.className);
      if (params?.batch) result = result.filter((r) => r.batch === params.batch);
...
export function useAddInstallment() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const orgId = userProfile?.orgId;
      if (!orgId) throw new Error("No org");
      const feeRef = orgDocRef(orgId, "fees", feeId);
      const feeSnap = await getDoc(feeRef);
      if (!feeSnap.exists()) throw new Error("Fee not found");
      const feeData = feeSnap.data() as any;
      const existing: any[] = feeData.installments ?? [];
      const inst = {
        amount,
        note: note ?? "",
        paidAt: new Date().toISOString(),
        collectedBy: userProfile?.name ?? userProfile?.email ?? "Admin",
      };
      const updated = [...existing, inst];
      const totalPaid = updated.reduce((s, i) => s + Number(i.amount), 0);
      const totalAmount = Number(feeData.amount);
      const newStatus =
        totalPaid >= totalAmount ? "paid" : totalPaid > 0 ? "partial" : "pending";
      await updateDoc(feeRef, {
        installments: updated,
        totalPaid,
        status: newStatus,
        paidAt: newStatus === "paid" ? serverTimestamp() : feeData.paidAt ?? null,
      });
    },
    onSuccess: () =>
      qc.invalidateQueries({ queryKey: [userProfile?.orgId, "fees"] }),
  });
}
...
export function useGetIncomeSummary() {
  ...
  const orgId = userProfile?.orgId;
  return useQuery({
    queryKey: [orgId, "fees", "income-summary"],
    queryFn: async () => {
      if (!orgId) return [];
      const snap = await getDocs(orgCol(orgId, "fees"));
      const rows = snap.docs.map(mapDoc).map((r: any) => ({
        ...r,
        amount: Number(r.amount),
        totalPaid: Number(
          r.totalPaid ?? (r.status === "paid" ? r.amount : 0),
        ),
      }));
      ...
      for (const r of rows) {
        const m: string =
          r.month ?? (r.createdAt as string)?.slice(0, 7) ?? "unknown";
        if (!grouped[m])
          grouped[m] = {
            month: m,
            billed: 0,
        ...
        grouped[m].billed += r.amount;
        grouped[m].collected += r.totalPaid;
        grouped[m].count++;
        if (r.status === "paid") grouped[m].paid++;
        else if (r.status === "partial") grouped[m].partial++;
        else grouped[m].pending++;
      }

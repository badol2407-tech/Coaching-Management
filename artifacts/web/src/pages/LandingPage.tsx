...
function CtaDashboardPreview() {
  return (
      <Card className="browser-mockup glass-panel-dark relative overflow-hidden border-sidebar-border shadow-2xl" data-testid="cta-dashboard-preview">
       <div className="browser-chrome flex items-center justify-between border-b border-sidebar-border px-4 py-3">
        <div className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-destructive/70" /><span className="h-2 w-2 rounded-full bg-chart-3/70" /><span className="h-2 w-2 rounded-full bg-primary/70" /></div>
        <Badge variant="secondary">Today at a glance</Badge>
      </div>
      <CardContent className="space-y-4 p-4">
        <div className="grid grid-cols-3 gap-2">{[["Students", "1,248"], ["Present", "92.6%"], ["Collected", "৳8.42L"]].map(([label, value]) => <div key={label} className="rounded-lg border border-sidebar-border bg-sidebar/50 p-3"><p className="text-xs text-sidebar-foreground/60">{label}</p><p className="mt-2 text-base font-semibold">{value}</p></div>)}</div>
        <div className="rounded-lg border border-sidebar-border bg-sidebar/50 p-3"><div className="flex items-center justify-between"><div><p className="text-xs font-semibold">Weekly attendance</p><p className="text-xs text-sidebar-foreground/60">One clear signal for your team</p></div><TrendingUp className="h-4 w-4 text-primary" aria-hidden="true" /></div><MiniSparkline /></div>
      </CardContent>
    </Card>
  );
}
    ...
    <div className="landing-shell min-h-screen overflow-x-clip bg-background text-foreground" id="top">
      ...
      <main>
        ...
        <section className="border-b px-4 py-8 sm:px-6 lg:px-8" aria-label="Trusted by school teams" data-testid="section-trusted-by"><div className="mx-auto flex max-w-7xl flex-col items-center gap-5 sm:flex-row sm:justify-between"><p className="text-center text-xs font-medium uppercase tracking-widest text-muted-foreground sm:text-left">Trusted by thoughtful education teams</p><div className="grid w-full grid-cols-2 gap-3 text-center sm:flex sm:w-auto sm:items-center sm:gap-8">{["Northbridge Academy", "BrightPath", "The Learning Room", "Cedar Grove"].map((name) => <span data-testid={`text-trusted-${name.toLowerCase().replace(/\s+/g, "-")}`} key={name} className="text-sm font-semibold tracking-tight text-muted-foreground/80">{name}</span>)}</div></div></section>
         ...
         <section id="overview" className="surface-blue scroll-mt-20 border-b px-4 py-20 sm:px-6 lg:px-8 lg:py-32" data-testid="section-overview"><div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-12 lg:items-center"><div className="lg:col-span-5"><SectionHeading eyebrow="The problem, made visible" title="Less tab-switching. More learning." description="When attendance lives in one sheet, fees in another, and updates in a message thread, good teams lose time to coordination. EduTrack gives the whole day a shared shape." align="left" /><div className="mt-8 flex flex-wrap gap-2"><Badge variant="outline">Admin</Badge><Badge variant="outline">Teacher</Badge><Badge variant="outline">Parent</Badge><Badge variant="outline">Student</Badge></div></div><div className="grid gap-4 sm:grid-cols-2 lg:col-span-7">{whyEduTrackPoints.map(({ icon: Icon, title, desc }, index) => <motion.div key={title} initial={reduceMotion ? false : { opacity: 0, y: 12 }} whileInView={reduceMotion ? undefined : { opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }}><Card data-testid={`card-overview-point-${index}`} className="h-full rounded-2xl border-primary/10 bg-background/75 p-6 shadow-sm transition-transform hover:-translate-y-1 hover:border-primary/40"><div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-primary/10 text-primary shadow-inner"><Icon className="h-5 w-5" aria-hidden="true" /></div><h3 className="mt-5 font-semibold">{title}</h3><p className="mt-2 text-sm leading-relaxed text-muted-foreground">{desc}</p></Card></motion.div>)}</div></div></section>

         <section id="features" className="scroll-mt-20 border-b bg-background px-4 py-20 sm:px-6 lg:px-8 lg:py-32" data-testid="section-features"><div className="mx-auto max-w-7xl"><SectionHeading eyebrow="Everything connected" title="The details that keep a school moving." description="A complete operational layer for attendance, fees, learning, logistics, and the people who make progress happen." /><div className="mt-14 grid grid-cols-4 gap-4 md:grid-cols-8 lg:grid-cols-12">{features.map(({ icon: Icon, title, desc, label }, index) => <motion.div key={title} className={index === 0 ? "col-span-4 md:col-span-4 lg:col-span-4" : index === 1 ? "col-span-4 md:col-span-4 lg:col-span-4" : "col-span-4 md:col-span-4 lg:col-span-3"} initial={reduceMotion ? false : { opacity: 0, y: 12 }} whileInView={reduceMotion ? undefined : { opacity: 1, y: 0 }} whileHover={reduceMotion ? undefined : { y: -6, scale: 1.01 }} viewport={{ once: true, amount: 0.15 }}><Card data-testid={`card-feature-${index}`} className="group h-full rounded-2xl bg-gradient-to-br from-card via-card to-primary/[0.03] p-6 shadow-sm transition-all hover:border-primary/40 hover:shadow-xl"><div className="flex items-start justify-between gap-3"><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-primary/25 via-primary/10 to-accent text-primary shadow-sm transition-transform group-hover:scale-110"><Icon className="h-5 w-5" aria-hidden="true" /></div><span className="text-xs text-muted-foreground">{label}</span></div><h3 className="mt-6 text-lg font-semibold">{title}</h3><p className="mt-2 text-sm leading-relaxed text-muted-foreground">{desc}</p><div className="mt-6 flex items-center gap-2 text-xs font-medium text-primary">Explore module <ArrowRight className="h-3 w-3 transition-transform group-hover:translate-x-1" aria-hidden="true" /></div></Card></motion.div>)}</div></div></section>

         <section id="solutions" className="surface-blue scroll-mt-20 border-b px-4 py-20 sm:px-6 lg:px-8 lg:py-32" data-testid="section-solutions"><div className="mx-auto max-w-7xl"><div className="grid gap-10 lg:grid-cols-12 lg:items-end"><SectionHeading eyebrow="Solutions for every role" title="One school. Four focused experiences." description="The same trustworthy record, shaped into a view that helps each person do their best work." align="left" /><div className="lg:col-span-4 lg:justify-self-end"><Button data-testid="button-solutions-demo" variant="outline" onClick={() => openAuth("login", "solutions")}>See the workspace <ArrowRight aria-hidden="true" /></Button></div></div><div className="mt-14 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{roleModules.map(({ icon: Icon, role, title, desc }, index) => <motion.div key={role} initial={reduceMotion ? false : { opacity: 0, y: 12 }} whileInView={reduceMotion ? undefined : { opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }}><Card data-testid={`card-solution-${index}`} className="h-full rounded-2xl border-primary/10 bg-background/70 p-6 shadow-sm"><div className="flex items-center justify-between gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-primary/10 text-primary"><Icon className="h-5 w-5" aria-hidden="true" /></div><Badge variant="secondary">{role}</Badge></div><h3 className="mt-6 text-lg font-semibold leading-tight">{title}</h3><p className="mt-3 text-sm leading-relaxed text-muted-foreground">{desc}</p></Card></motion.div>)}</div></div></section>

         <section className="border-b bg-background px-4 py-20 sm:px-6 lg:px-8 lg:py-32" data-testid="section-dashboard"><div className="mx-auto max-w-7xl"><div className="grid gap-12 lg:grid-cols-12 lg:items-center"><div className="lg:col-span-4"><SectionHeading eyebrow="The daily command center" title="A dashboard that answers before you ask." description="Attendance, fees, results, assignments, notices, library, transport, and analytics — visible as one connected operating picture." align="left" /><div className="mt-7 flex flex-wrap gap-2"><Badge variant="secondary">Attendance</Badge><Badge variant="secondary">Fees</Badge><Badge variant="secondary">Results</Badge><Badge variant="secondary">Analytics</Badge></div></div><div className="lg:col-span-8"><DashboardShowcase variant="command" /></div></div></div></section>

         <section className="surface-lavender border-b px-4 py-20 sm:px-6 lg:px-8 lg:py-32" data-testid="section-workflow"><div className="mx-auto max-w-7xl"><SectionHeading eyebrow="The connected workflow" title="From Admin to Student, nothing gets lost in translation." description="Each update travels with the people who need it — no duplicate spreadsheets, no guessing which message is current." /><div className="relative mt-14 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{workflow.map(({ num, role, icon: Icon, title, desc }, index) => <motion.div key={role} initial={reduceMotion ? false : { opacity: 0, y: 12 }} whileInView={reduceMotion ? undefined : { opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.2 }} className="relative"><Card data-testid={`card-workflow-${index}`} className="h-full rounded-2xl bg-background/75 p-6 shadow-sm"><div className="flex items-center justify-between"><span className="font-mono text-sm text-primary">{num}</span><Icon className="h-5 w-5 text-primary" aria-hidden="true" /></div><Badge variant="outline" className="mt-6">{role}</Badge><h3 className="mt-4 text-lg font-semibold">{title}</h3><p className="mt-2 text-sm leading-relaxed text-muted-foreground">{desc}</p></Card>{index < workflow.length - 1 && <ArrowRight className="absolute -right-3 top-1/2 z-10 hidden h-5 w-5 -translate-y-1/2 text-primary lg:block" aria-hidden="true" />}</motion.div>)}</div></div></section>

         <section className="border-b bg-background px-4 py-20 sm:px-6 lg:px-8 lg:py-32" data-testid="section-showcase"><div className="mx-auto max-w-7xl"><SectionHeading eyebrow="See the system in context" title="Built for the minutes between the big moments." description="The value is not another dashboard. It is the confidence that the right record is already there when your team needs it." /><div className="mt-14"><DashboardShowcase variant="context" /></div></div></section>

         <section className="surface-blue border-b px-4 py-20 sm:px-6 lg:px-8 lg:py-32" data-testid="section-statistics"><div className="mx-auto max-w-7xl"><div className="grid gap-10 md:grid-cols-[1.05fr_2fr] md:items-end"><SectionHeading eyebrow="The signal adds up" title="A clearer day compounds over time." description="Small operational wins become measurable progress when every role works from the same current record." align="left" /><p className="max-w-sm text-sm leading-relaxed text-muted-foreground md:justify-self-end">Designed for school and coaching centre teams who need fewer follow-ups and more confidence in the next decision.</p></div><div className="mt-14 grid gap-4 sm:grid-cols-3">{[["25K+", "Schools and coaching centres"], ["95%", "Attendance accuracy"], ["10M+", "Records processed"]].map(([value, label], index) => <motion.div key={label} initial={reduceMotion ? false : { opacity: 0, y: 14 }} whileInView={reduceMotion ? undefined : { opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.3 }} transition={{ delay: index * .08 }} className="rounded-2xl border border-primary/10 bg-background/65 p-6 shadow-sm sm:p-8" data-testid={`statistic-${index}`}><p className="landing-stat font-display text-5xl text-primary sm:text-6xl">{value}</p><p className="mt-3 max-w-[12rem] text-sm leading-relaxed text-muted-foreground">{label}</p></motion.div>)}</div></div></section>

        <TestimonialsSection />

         <section id="pricing" className="scroll-mt-20 border-b bg-background px-4 py-20 sm:px-6 lg:px-8 lg:py-32" data-testid="section-pricing"><div className="mx-auto max-w-7xl"><SectionHeading eyebrow="Pricing that grows with you" title="Start small. Stay in control." description="কোনো hidden charge নেই। আপনার school বা coaching center-এর stage অনুযায়ী plan বেছে নিন।" /><div className="mt-14 grid gap-5 lg:grid-cols-3">{(["free_trial", "founder_launch", "annual_premium"] as PlanTier[]).map((tier) => <PricingCard key={tier} tier={tier} onSelect={selectPlan} />)}</div><div className="mt-8 flex flex-wrap items-center justify-center gap-x-6 gap-y-2 text-sm text-muted-foreground">{["কোনো hidden fee নেই", "যেকোনো সময় cancel করুন", "Role-based access"].map((signal) => <span key={signal} className="flex items-center gap-2"><CheckCircle className="h-4 w-4 text-primary" aria-hidden="true" />{signal}</span>)}</div></div></section>
         ...
         <section className="landing-cta relative overflow-hidden border-b px-4 py-20 text-sidebar-foreground sm:px-6 lg:px-8 lg:py-32" data-testid="section-final-cta"><div className="pointer-events-none absolute -left-24 -top-24 h-96 w-96 rounded-full bg-primary/20 blur-3xl" aria-hidden="true" /><div className="pointer-events-none absolute -bottom-32 -right-24 h-96 w-96 rounded-full bg-chart-4/20 blur-3xl" aria-hidden="true" /><div className="relative mx-auto grid max-w-7xl gap-14 lg:grid-cols-12 lg:items-center"><div className="lg:col-span-5"><Badge variant="secondary" className="mb-4">৭ দিনের Free Trial</Badge><h2 className="font-display text-3xl leading-tight tracking-tight sm:text-5xl">Ready to modernize <span className="text-primary">your school?</span></h2><p className="mt-4 max-w-xl text-base leading-relaxed text-sidebar-foreground/75">আপনার team-এর জন্য একটি dependable workspace তৈরি করুন — আজই শুরু করুন, প্রথম দিন থেকেই clarity পান।</p><div className="mt-7 flex flex-col gap-3 sm:flex-row"><Button data-testid="button-final-start" size="lg" className="glow-primary" onClick={() => openAuth("login", "final_cta")}>ফ্রি ট্রায়াল শুরু করুন <ArrowRight aria-hidden="true" /></Button><Button data-testid="button-final-demo" size="lg" variant="outline" className="bg-background/10 text-sidebar-foreground hover:bg-background/20" onClick={() => openAuth("login", "final_cta_demo")}>লাইভ ডেমো দেখুন <CalendarCheck aria-hidden="true" /></Button></div></div><div className="lg:col-span-7"><CtaDashboardPreview /></div></div></section>
      </main>

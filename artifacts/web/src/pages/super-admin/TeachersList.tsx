...
import { Search, School, MailCheck, Trash2, ShieldOff, ShieldCheck } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">All teacher accounts across the platform</p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <Card><CardContent className="pt-5"><p className="text-sm text-muted-foreground">Total Teachers</p><p className="text-3xl font-bold mt-1">{teachers.length}</p></CardContent></Card>
        <Card><CardContent className="pt-5"><p className="text-sm text-muted-foreground">Active</p><p className="text-3xl font-bold mt-1 text-green-500">{teachers.filter((u: any) => !u.disabled).length}</p></CardContent></Card>
        <Card><CardContent className="pt-5"><p className="text-sm text-muted-foreground">Disabled</p><p className="text-3xl font-bold mt-1 text-red-500">{teachers.filter((u: any) => u.disabled).length}</p></CardContent></Card>
      </div>

      <Card>
        <CardHeader className="pb-3">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            <CardTitle className="text-base flex items-center gap-2"><School className="h-4 w-4" aria-hidden="true" /> Teacher Accounts</CardTitle>
            <div className="relative sm:ml-auto w-full sm:w-64">
              <Search className="absolute left-2.5 top-2.5 h-3.5 w-3.5 text-muted-foreground" aria-hidden="true" />
              <Input aria-label="Search teacher accounts" placeholder="Search by name, email…" className="pl-8 h-10 text-sm" value={search} onChange={(e) => setSearch(e.target.value)} />
            </div>
          </div>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            ...
            <p className="text-center text-muted-foreground py-10 text-sm" role="status" aria-live="polite">No teachers found</p>
          ) : (
                    ...
                    <tr key={u.uid} className="border-b border-border/50 hover:bg-accent/30 transition-colors">
                      ...
                      <td className="py-2.5 text-right">
                        <div className="flex items-center justify-end gap-1">
                          <Button size="icon" variant="ghost" className={actionButtonClass} aria-label={`Send password reset to ${u.email}`} title="Send password reset" onClick={() => handleResetPassword(u.email)}><MailCheck className="h-3.5 w-3.5" aria-hidden="true" /></Button>

...
function downloadTemplate() {
  const headers = ["Name *", "Phone", "Email", "Class", "Guardian Name", "Guardian Phone", "Enrolled Date (YYYY-MM-DD)", "Roll Number", "Status", "Emergency Contact", "Emergency Phone", "Photo URL"];
  const sample = [
    ["Rahim Uddin", "01711-123456", "rahim@example.com", "Class 9", "Karim Uddin", "01711-654321", "2025-01-15", "001", "active", "Uncle Karim", "01711-654322", ""],
    ["Nasrin Akter", "01812-234567", "nasrin@example.com", "Class 10", "Abdur Rahim", "01812-765432", "2025-02-01", "002", "active", "Aunt Fatima", "01812-765433", ""],
  ];
...
function ManualAdd() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const studentDocRef = await addDoc(collection(db, "organizations", userProfile.orgId, "students"), {
        uid,
        name: form.name.trim(),
    ...
    return (
      <div className="py-14 text-center space-y-5">
        ...
        <div className="flex gap-3 justify-center">
          <Button variant="outline" onClick={() => { setDone(false); setShowPassword(false); setForm({ name: "", phone: "", email: "", password: "", className: "", section: "", batch: "", guardianName: "", guardianPhone: "", enrolledAt: new Date().toISOString().split("T")[0], address: "", status: "active", rollNumber: "", emergencyContact: "", emergencyPhone: "", photoUrl: "" }); }}>
            Add Another
          </Button>
    ...
    <form onSubmit={handleSubmit} className="space-y-5" aria-describedby="manual-form-guidance">
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        ...
        <div className="space-y-1.5 sm:col-span-2">
          ...
          <Input id="m-address" placeholder="Residential address" value={form.address ?? ""}
            onChange={e => { setForm(f => ({ ...f, address: e.target.value })); }}
            className="" />
        </div>
        <div className="space-y-1.5">
          <Label htmlFor="m-rollno">Roll Number</Label>
          <Input id="m-rollno" placeholder="e.g., 001, A-15" value={form.rollNumber}
            onChange={e => { setForm(f => ({ ...f, rollNumber: e.target.value })); setErrors(er => ({ ...er, rollNumber: false })); }}
        ...
        <div className="space-y-1.5">
          ...
          <Select value={form.status} onValueChange={(val) => setForm(f => ({ ...f, status: val as "active" | "inactive" }))}>
            <SelectTrigger id="m-status"><SelectValue placeholder="Select status" /></SelectTrigger>
            <SelectContent>
        ...
        <div className="space-y-1.5">
          <Label htmlFor="m-emerg-contact">Emergency Contact Name</Label>
          <Input id="m-emerg-contact" placeholder="Contact person name" value={form.emergencyContact}
            onChange={e => { setForm(f => ({ ...f, emergencyContact: e.target.value })); setErrors(er => ({ ...er, emergencyContact: false })); }}
            className={errors.emergencyContact ? "border-destructive focus-visible:ring-destructive" : ""} />
        </div>
        <div className="space-y-1.5">
          <Label htmlFor="m-emerg-phone">Emergency Contact Phone</Label>
          <Input id="m-emerg-phone" placeholder="01XXXXXXXXX" value={form.emergencyPhone}
            onChange={e => { setForm(f => ({ ...f, emergencyPhone: e.target.value })); setErrors(er => ({ ...er, emergencyPhone: false })); }}
            className={errors.emergencyPhone ? "border-destructive focus-visible:ring-destructive" : ""} />
        </div>
        <div className="space-y-1.5 sm:col-span-2">
          <Label htmlFor="m-photo">Student Photo URL</Label>
          <Input id="m-photo" placeholder="https://example.com/photo.jpg" value={form.photoUrl}
            onChange={e => { setForm(f => ({ ...f, photoUrl: e.target.value })); setErrors(er => ({ ...er, photoUrl: false })); }}
            className={errors.photoUrl ? "border-destructive focus-visible:ring-destructive" : ""} />
          <p className="text-xs text-muted-foreground">Paste a direct image URL (optional)</p>
        </div>
      </div>
      ...
      try {
        ...
        const parsed: ImportRow[] = json.slice(1).filter(r => r[0]?.toString().trim()).map((r) => {
          ...
          const row: ImportRow = {
            ...
            // Sprint 1 additions
            rollNumber: r[7]?.toString().trim() || undefined,
            status: (r[8]?.toString().trim() as "active" | "inactive" | undefined) || "active",
            emergencyContact: r[9]?.toString().trim() || undefined,
            emergencyPhone: r[10]?.toString().trim() || undefined,
            photoUrl: r[11]?.toString().trim() || undefined,
          };
          if (!name) {
            row._status = "error";
...
function AdmissionLinkTab() {
  ...
  async function approve(req: AdmissionRequest) {
    ...
    try {
      const studentDocRef = await addDoc(collection(db, "organizations", orgId, "students"), {
        uid: req.uid ?? null,
        name: req.name,
        ...
        // Sprint 1 additions (default values for admission link)
        status: "active",
        rollNumber: null,

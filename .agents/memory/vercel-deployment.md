...
and reference their SHAs in the deployment body. The uploader also builds Flowora
and places it under `/flowora/` in that same static output.
...
## vercel.json
Points to `artifacts/web` build: `pnpm --filter @workspace/web run build`, output `artifacts/web/dist/public`. The custom uploader additionally builds `artifacts/flowora-onboarding` with `BASE_PATH=/flowora/`.

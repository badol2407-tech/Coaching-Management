...
**How to apply:** Ensure `FIREBASE_API_KEY` is available to the development and production build environments, keep the Firebase project metadata consistent across auth clients, and validate with typecheck, build, workflow health, and viewport screenshots. EduTrack fresh browser sessions intentionally show a splash screen and may show the session-scoped promotion popup before revealing the landing page, so let both settle when validating screenshots without changing production behavior.
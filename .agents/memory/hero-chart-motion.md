...
The four hero windows should stay hidden below the stage until the first-session promotion is dismissed, then rise smoothly from below into their existing floating positions without rotating the windows. Store the completion in session storage so refreshes do not replay it.
